...
save(scRNAsub, file = "Subset_CD8+T2/scRNAsub_harmony.Rdata")
...
write.csv(ClusterMarker,'Subset_CD8+T2/ClusterMarker.csv', row.names=F)
top = 5   #可根据需要调整
...
ggsave("Subset_CD8+T2/Markers_heatmap.png", width = 7, height = 3.5)

p <- VlnPlot(scRNAsub, features = markers, stack = T, split.by = 'orig')
ggsave('Subset_CD8+T2/vlnplot.png',p, width = 7, height = 3.5 )
